...
const COLLECTION = "yearlyCategories";
...
export async function getYearlyCategories(uid: string): Promise<ExpenseCategory[]> {
  const { getDocs } = await import("firebase/firestore");
  const snap = await getDocs(col(uid));
  return snap.docs.map((d) => toCategory(d.id, d.data()));
}

export function subscribeYearlyCategories(
  uid: string,
...
export async function addYearlyCategory(
  uid: string,
...
export async function updateYearlyCategory(
  uid: string,
  id: string,
  data: { name: string; icon: string; gradientPreset: string }
): Promise<void> {
...
export async function deleteYearlyCategory(uid: string, id: string): Promise<void> {
