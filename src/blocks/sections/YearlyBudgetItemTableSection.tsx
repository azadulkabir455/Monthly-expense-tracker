...
import { EditYearlyBudgetItemModal } from "@/blocks/components/EditYearlyBudgetItemModal";
import { Skeleton } from "@/blocks/elements/Skeleton";

interface YearlyBudgetItemTableSectionProps {
  year: number;
  categoryId?: string;
  items: YearlyBudgetItem[];
  loading?: boolean;
  onDeleteItem: (id: string) => Promise<void>;
  onUpdateItem: (item: YearlyBudgetItem) => Promise<void>;
  headerRight?: React.ReactNode;
}

export function YearlyBudgetItemTableSection({
  year,
  categoryId,
  items,
  loading = false,
  ...
  onUpdateItem,
  headerRight,
}: YearlyBudgetItemTableSectionProps) {
  const { theme } = useThemeContext();
  const isDark = theme === "dark";
  const { t } = useLanguage();
  const [deleteTarget, setDeleteTarget] = useState<YearlyBudgetItem | null>(null);
  const [confirmModalOpen, setConfirmModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<YearlyBudgetItem | null>(null);
  ...
  useEffect(() => {
    function handleClickOutside(e: MouseEvent) {
      ...
      if (!target.closest("[data-yearly-budget-action-menu]")) setOpenActionId(null);
    }
  ...
  const handleEdit = (item: YearlyBudgetItem) => {
    setEditingItem(item);
    setEditModalOpen(true);
  };

  const handleDelete = (item: YearlyBudgetItem) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast.success(t("yearlyBudget_deleted"));
  ...
  return (
    <SectionCard>
      <SectionHeader>
        <div>
          <SectionTitle>{t("yearlyBudget_listTitle")}</SectionTitle>
          <SectionSubtitle>{t("yearlyBudget_listSubtitle")}</SectionSubtitle>
        </div>
        {headerRight}
      </SectionHeader>
              ...
              {items.map((item) => (
                ...
                >
                  ...
                  <td className="px-2 py-3">
                    <div data-yearly-budget-action-menu className="relative flex justify-end gap-1">
      ...
      <ConfirmModal
        ...
        title={t("yearlyBudget_deleteTitle")}
        message={deleteTarget ? t("yearlyBudget_deleteMessage", { name: deleteTarget.name }) : ""}
        confirmLabel={t("yearlyBudget_deleteConfirm")}
        cancelLabel={t("yearlyBudget_deleteCancel")}
      />
    </SectionCard>
  );
}
