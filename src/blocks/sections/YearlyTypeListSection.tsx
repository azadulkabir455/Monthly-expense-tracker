...
interface YearlyTypeListSectionProps {
  selectedCategoryId: string;
}

export function YearlyTypeListSection({
  selectedCategoryId,
}: YearlyTypeListSectionProps) {
  const { theme } = useThemeContext();
  const isDark = theme === "dark";
  const { categories, loading: categoriesLoading } = useYearlyCategories();
  const { types: allTypes, loading: typesLoading, deleteType, updateType } = useYearlyTypes();
  const { t } = useLanguage();
  ...
  useEffect(() => {
    function handleClickOutside(e: MouseEvent) {
      ...
      if (!target.closest("[data-yearly-type-action-menu]")) setOpenActionId(null);
    }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast.success(t("yearlyType_deleted"));
  ...
  return (
    <SectionCard>
      <SectionHeader>
        <div>
          <SectionTitle>{t("yearlyType_listTitle")}</SectionTitle>
          <SectionSubtitle>{t("yearlyType_listSubtitle")}</SectionSubtitle>
        </div>
      </SectionHeader>

      {(categoriesLoading || typesLoading) ? (
        <ul className="space-y-2 sm:space-y-3 overflow-visible">
          {Array.from({ length: Math.max(types.length || allTypes.length || 0, 2) }, (_, i) => (
            <li
              key={i}
              className={cn(
      ...
      <ul className="space-y-2 sm:space-y-3 overflow-visible">
        {types.map((typeItem) => {
          ...
          >
            ...
            <div data-yearly-type-action-menu className="relative flex shrink-0 items-center gap-1">
      ...
      {!categoriesLoading && !typesLoading && types.length === 0 && (
        <p className={cn("py-8 text-center text-sm", isDark ? "text-slate-400" : "text-slate-500")}>
          {selectedCategoryId ? t("yearlyType_emptyInCategory") : t("yearlyType_emptyGlobal")}
        </p>
      )}
      ...
      <ConfirmModal
        ...
        title={t("yearlyType_deleteTitle")}
        message={deleteTarget ? t("yearlyType_deleteMessage", { name: deleteTarget.name }) : ""}
        confirmLabel={t("yearlyType_deleteConfirm")}
        cancelLabel={t("yearlyType_deleteCancel")}
      />
    </SectionCard>
  );
}
